...
impl VirtualStoreLayout {
    ...
    /// [`Self::new`], with the derived suffix map cached on disk under
    /// the identity of the lockfile file it was read from.
    ///
    /// Only for a caller whose `snapshots` and `packages` are the
    /// unmodified contents of the lockfile in `lockfile_dir` — the
    /// restore path. The cache is keyed on that file, so a caller
    /// holding a freshly resolved lockfile would be served the map of
    /// the one still on disk, and every package the resolve added would
    /// miss the map and fall back to a flat-named slot.
    ///
    /// The cache module below documents what the key covers.
    #[must_use]
    pub fn new_from_lockfile_file(
        config: &Config,
        engine: Option<&str>,
        snapshots: Option<&HashMap<PackageKey, SnapshotEntry>>,
        packages: Option<&HashMap<PackageKey, PackageMetadata>>,
        ...
        let cache_key = config.enable_global_virtual_store.then(|| {
            gvs_layout_cache::fingerprint(gvs_layout_cache::Inputs {
                lockfile_dir,
                snapshots,
                engine,
                allow_build_policy,
        ...
        let layout =
            Self::new(config, engine, snapshots, packages, allow_build_policy, lockfile_dir);
        if let Some(key) = cache_key.as_deref()
...
/// skipped check:
///
///   * the wanted lockfile, digested over its bytes. Snapshots and
///     package metadata both come from it, and only content identifies
///     it: a same-length in-place rewrite inside one filesystem mtime
///     tick leaves `(len, mtime, inode)` untouched. Hashing the bytes
///     also makes a fresh checkout or CI worktree hit the cache rather
///     than re-derive;
///   * the snapshot keys the caller actually holds, in lockfile order.
///     [`VirtualStoreLayout::new_from_lockfile_file`] is documented for
///     callers whose snapshots are that file unmodified, and this is
///     what holds them to it: a caller that reached the restore path
///     with a lockfile amended in memory gets a miss rather than a map
///     that is missing the amended entries;
...
///   * a format version, so a change to the hash payload or to this
///     file's encoding retires previous entries.
mod gvs_layout_cache {
    ...
    /// What [`fingerprint`] digests. Named because the suffix map is
    /// only as safe to reuse as this list is complete.
    #[derive(Clone, Copy)]
    pub(super) struct Inputs<'a> {
        pub lockfile_dir: Option<&'a Path>,
        pub snapshots: Option<&'a HashMap<PackageKey, SnapshotEntry>>,
        pub engine: Option<&'a str>,
        pub allow_build_policy: Option<&'a AllowBuildPolicy>,
    }

    /// Digest of every input the suffix map is derived from, or `None`
    /// when there is no lockfile to read — in which case nothing is
    /// cached and the map is derived as before.
    pub(super) fn fingerprint(inputs: Inputs<'_>) -> Option<String> {
        let dir = inputs.lockfile_dir?;
        let lockfile = std::fs::read(dir.join(pnpm_lockfile::Lockfile::FILE_NAME)).ok()?;
        ...
        if let Some(snapshots) = inputs.snapshots {
            hasher.update((snapshots.len() as u64).to_le_bytes());
            for (snapshot_key, _) in crate::deps_graph::in_lockfile_order(snapshots) {
                let key = snapshot_key.to_string();
                hasher.update((key.len() as u64).to_le_bytes());
                hasher.update(key.as_bytes());
            }
        }
        hasher.update(inputs.engine.unwrap_or("").as_bytes());
        hasher.update(
            inputs
                .allow_build_policy
                .map(AllowBuildPolicy::fingerprint)
                .unwrap_or_default()
                .as_bytes(),
        );
